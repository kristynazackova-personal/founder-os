...
const BUILD_FIELDS: PmfFieldDef[] = [
  { key: "one_sentence", stage: "goal", label: "In one sentence, what does this product do", prompt: "The thing itself, not the category. Mechanism, not benefit." },
  ...
  { key: "top_metric", stage: "metrics", label: "Top metric for it", prompt: "The one number closest to that impact." },
  { key: "retention_targets", stage: "metrics", label: "Retention: week 1, month 1, 90 days", prompt: "What share of users you expect back at each horizon, and what would worry you." },
  { key: "early_signals", stage: "metrics", label: "Other early signals", prompt: "What you will see before retention data exists: signups, thumbs up, thank-yous." },
  { key: "jobs", stage: "jtbd", label: "The user must be able to…", prompt: "One per line, naming the specific user: 'As a <specific user>, I want to <do>, so that <outcome>'.", long: true },
  { key: "jobs_implications", stage: "jtbd", label: "What each job implies for the build", prompt: "The functionality each line forces, which is what you hand an LLM.", long: true },
  { key: "platform_first", stage: "platform", label: "Which platform first, and why", prompt: "One platform. Reach against how long it takes you to build." },
  { key: "wedge", stage: "platform", label: "Acquisition wedge vs main home", prompt: "Whether one surface exists to be easy to trust and funnel into the richer one." },
  { key: "design_guidelines", stage: "platform", label: "Design guidelines to hand over", prompt: "Delight and gamification, the core interaction, auth and hierarchy, and the must-haves for your specific user.", long: true },
  { key: "prompt_slots", stage: "engine", label: "Prompt slots", prompt: "Role the AI plays, audience and their needs, what triggers it, what it produces, tone.", long: true },
  ...
  { key: "research_plan", stage: "validate", label: "Cheap research you will actually do", prompt: "Which groups, which threads, which people you can reach this week." },
  { key: "gate", stage: "validate", label: "The decision gate", prompt: "Four answers: top pain confirmed by real users, output genuinely helped, v1 scoped to one segment and 1-3 pains and one platform, metrics defined and trackable.", long: true },
];
