...
/** One row per campaign that has spend, clicks or attributed events; "(not set)" (organic / unattributed) is dropped. Sorted by spend. */
export function summarizeCampaigns(ads: CampaignAdRow[], events: CampaignEventRow[], opts: { monthlyRevenuePerPayingCents: number | null }): { rows: CampaignSummary[]; total: CampaignSummary } {
  ...
  for (const a of ads) {
    if (isUnsetCampaign(a.campaign)) continue;
    const row = get(a.campaign);
    row.spendCents += a.costCents;
    row.clicks += a.clicks;
  }
  for (const e of events) {
    if (isUnsetCampaign(e.campaign)) continue;
    const row = get(e.campaign);
  ...
  const rows = [...map.values()].map(finish).sort((a, b) => b.spendCents - a.spendCents || b.installs - a.installs);
  const total = finish(
    rows.reduce(
